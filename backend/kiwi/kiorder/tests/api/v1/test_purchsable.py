...
from kiorder.models import Purchasable, PurchasableOption, PurchasableCategory
...
def test_get_purchsable(client, store):
    ...
    with patch('kiorder.api.v1.base.Store') as Store, \
        ...
        assert response.json() == {'success': True, 'data': {'list': [{'name': 'CAT', 'purchasables': [{'id': 1, 'name': 'Purchasable With options 1', 'thumbnail': None, 'base_price': '10000.0000000000', 'options': [{'id': 1, 'name': 'Option 1', 'base_price': '1000.0000000000', 'max_capacity': 2}]}]}]}}


        response = client.get(f'/kiorder/api/v1/purchasable/{purchasable.id}')
        assert response.status_code == 200
        assert response.json()['data'] == {'id': 1, 'name': 'Purchasable With options 1', 'thumbnail': None, 'base_price': '10000.0000000000', 'options': [{'id': 1, 'name': 'Option 1', 'base_price': '1000.0000000000', 'max_capacity': 2}]}
