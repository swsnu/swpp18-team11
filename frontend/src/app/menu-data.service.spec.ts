...
describe('MenuDataService', () => {
  ...
  it('getCategories should return stream of Categories', () => {
    let testCategory: Category = {name: 'Burgers', purchasables: []}
    let testCategories: Category[] = [testCategory]
    let testJSONResponse: any = {
  ...
  it('getProductInfo should return stream of Menu from given Id', () =>{
    let testMenu: any = {id: 1, name: 'Hamburger', thumbnail: 'jpg', base_price: 10, options: []}
    let menuUrl = purchasableUrl + '/' + testMenu.id
    let testJSONResponse: any = {
      "success": true,
